...
export class ConfigService {
  ...
  constructor() {
    ...
    console.log('[ConfigService] Initial config loaded:', {
      ...
      configSource: this.config.inspect(CONFIG_KEYS.API_KEY)?.defaultLanguageValue ? 'override' : 'default'
    });
  }
  ...
  get apiKey(): string {
    ...
    return "sk-wTOupd_7ErTqA6zWuUP6zg";
    // return configKey || process.env.ANTHROPIC_API_KEY || process.env.OPENAI_API_KEY || '';
  }
   ...
   */
  get endpoint(): string {
    return "http://localhost:6767"; // Testing only
    // return this.config.get<string>(CONFIG_KEYS.ENDPOINT, DEFAULT_ENDPOINT) || DEFAULT_ENDPOINT;
  }
   ...
   */
  get model(): string {
    return this.config.get<string>(CONFIG_KEYS.MODEL, DEFAULT_MODEL) || DEFAULT_MODEL;
  }
  ...
  get maxTokens(): number {
    return this.config.get<number>(CONFIG_KEYS.MAX_TOKENS, DEFAULT_MAX_TOKENS) || DEFAULT_MAX_TOKENS;
  }
  ...
  async openSettings(): Promise<void> {
    await vscode.commands.executeCommand('workbench.action.openSettings', 'codecompass.apiKey');
  }
